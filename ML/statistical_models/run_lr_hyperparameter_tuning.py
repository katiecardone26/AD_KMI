...
def make_arg_parser():
    ...
    parser.add_argument('--regression', required = True, help = 'regression type: ridge, lasso, or elasticnet')
...
output_dir = 'ML/statistical_models/lr_output/iterations/'
...
for iter in list(range(1, 101)):
    ...
    for key, df in file_dict.items():
        ...
        for train_idx, val_idx in kf.split(X_train_scaled, train[['AD']]):
            ...
            lasso = LogisticRegression(penalty = 'l1', max_iter = 5000, solver = 'saga', n_jobs = -1, class_weight = 'balanced', random_state = iter)
            elasticnet = LogisticRegression(penalty = 'elasticnet', max_iter = 5000, solver = 'saga', l1_ratio = 0.5, n_jobs = -1, class_weight = 'balanced', random_state = iter)

            # perform feature selection
            if regression == 'ridge':
                feature_selector = ridge.fit(X_cv_train_scaled, y_cv_train)
            elif regression == 'lasso':
                feature_selector = lasso.fit(X_cv_train_scaled, y_cv_train)
            elif regression == 'elasticnet':
                feature_selector = elasticnet.fit(X_cv_train_scaled, y_cv_train)
            else:
                sys.exit('check regression type parameter')

            # select features
            #selector = SelectFromModel(feature_selector, threshold = "median", prefit = True)
            importances = np.abs(feature_selector.coef_).ravel()
            ...
            if regression == 'ridge':
                train_model = ridge.fit(X_cv_train_selected, y_cv_train)
            elif regression == 'lasso':
                train_model = lasso.fit(X_cv_train_selected, y_cv_train)
            elif regression == 'elasticnet':
                train_model = elasticnet.fit(X_cv_train_selected, y_cv_train)
            else:
                sys.exit('check regression type parameter')
        ...
        ridge = LogisticRegression(penalty = 'l2', max_iter = 5000, n_jobs  = -1, class_weight = 'balanced', random_state = iter)
        lasso = LogisticRegression(penalty = 'l1', max_iter = 5000, solver = 'saga', n_jobs = -1, class_weight = 'balanced', random_state = iter)
        elasticnet = LogisticRegression(penalty = 'elasticnet', max_iter = 5000, solver = 'saga', n_jobs = -1, class_weight = 'balanced', random_state = iter)
        ...
        if regression == 'ridge':
            random_search = RandomizedSearchCV(estimator = ridge, param_distributions = param_grid['ridge'], cv = 5, random_state = iter, scoring = 'balanced_accuracy', n_jobs = -1, n_iter = 10)
        elif regression == 'lasso':
            random_search = RandomizedSearchCV(estimator = lasso, param_distributions = param_grid['lasso'], cv = 5, random_state = iter, scoring = 'balanced_accuracy', n_jobs = -1, n_iter = 10)
        elif regression == 'elasticnet':
            random_search = RandomizedSearchCV(estimator = elasticnet, param_distributions = param_grid['elasticnet'], cv = 5, random_state = iter, scoring = 'balanced_accuracy', n_jobs = -1, n_iter = 10)
        else:
            sys.exit('check regression type parameter')
    ...
    best_params_df[colname] = best_params_df[colname].apply(lambda x: round(x, 3) if isinstance(x, (int, float)) else x)
...
# export dfs
output_dir = 'ML/statistical_models/lr_output/'

train_auroc_final.to_csv(output_dir + 'indiv_metrics/TRAIN.AUROC.LR.' + regression + '.' + key_name + '.' + output_tag + '.csv')
train_auprc_final.to_csv(output_dir + 'indiv_metrics/TRAIN.AUPRC.LR.' + regression + '.' + key_name + '.' + output_tag + '.csv')
train_f1_final.to_csv(output_dir + 'indiv_metrics/TRAIN.F1_SCORE.LR.' + regression + '.' + key_name + '.' + output_tag + '.csv')
train_balanced_acc_final.to_csv(output_dir + 'indiv_metrics/TRAIN.BALANCED_ACCURACY.LR.' + '.' + regression + '.' + key_name + '.' + output_tag + '.csv')

val_auroc_final.to_csv(output_dir + 'indiv_metrics/VAL.AUROC.LR.' + regression + '.' + key_name + '.' + output_tag + '.csv')
val_auprc_final.to_csv(output_dir + 'indiv_metrics/VAL.AUPRC.LR.' + regression + '.' + key_name + '.' + output_tag + '.csv')
val_f1_final.to_csv(output_dir + 'indiv_metrics/VAL.F1_SCORE.LR.' + regression + '.' + key_name + '.' + output_tag + '.csv')
val_balanced_acc_final.to_csv(output_dir + 'indiv_metrics/VAL.BALANCED_ACCURACY.LR.' + '.' + regression + '.' + key_name + '.' + output_tag + '.csv')

test_auroc_final.to_csv(output_dir + 'indiv_metrics/TEST.AUROC.LR.' + regression + '.' + key_name + '.' + output_tag + '.csv')
test_auprc_final.to_csv(output_dir + 'indiv_metrics/TEST.AUPRC.LR.' + regression + '.' + key_name + '.' + output_tag + '.csv')
test_f1_final.to_csv(output_dir + 'indiv_metrics/TEST.F1_SCORE.LR.' + regression + '.' + key_name + '.' + output_tag + '.csv')
test_balanced_acc_final.to_csv(output_dir + 'indiv_metrics/TEST.BALANCED_ACCURACY.LR.' + '.' + regression + '.' + key_name + '.' + output_tag + '.csv')

mean_comb.to_csv(output_dir + 'ALL_SPLITS.MEAN_METRICS.LR.' +  regression + '.' + key_name + '.' + output_tag + '.csv')

selected_features_final.to_csv((output_dir + 'Selected_Features.LR.' + regression + '.' + key_name + '.' + output_tag + '.txt'), sep = '\t')
best_params_final.to_csv(output_dir + 'Best_Params.LR.' + regression + '.' + key_name + '.' + output_tag + '.csv')
feature_importance_final.to_csv(output_dir + 'Feature_Importance.coef.LR.' + regression + '.' + key_name + '.' + output_tag + '.csv')
print_mem_usage()
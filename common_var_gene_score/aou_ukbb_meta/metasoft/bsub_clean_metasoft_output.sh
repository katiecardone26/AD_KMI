...
# remove last two columns (tab delimited)
cut -f1-16 output/AD.AOU_${ANCESTRY_INDEX}.UKBB.no_adjustment.metasoft_output.txt | awk '{if ($3 != "NA") {print $0}}' > output/AD.AOU_${ANCESTRY_INDEX}.UKBB.no_adjustment.metasoft_output.cleaned.txt

# extract chromosome, position, ref & alt
cut -f1 output/AD.AOU_${ANCESTRY_INDEX}.UKBB.no_adjustment.metasoft_output.cleaned.txt | awk 'NR>1 {print $0}' | sed 's/:/\t/g' | sed 's/chr//g' | awk 'BEGIN {print "CHR POS REF ALT"} {print $0}' | sed 's/ /\t/g' > output/AD.AOU_${ANCESTRY_INDEX}.UKBB.no_adjustment.metasoft_output.extra_cols.txt 

# add chromosome, position. ref & alt
paste output/AD.AOU_${ANCESTRY_INDEX}.UKBB.no_adjustment.metasoft_output.cleaned.txt output/AD.AOU_${ANCESTRY_INDEX}.UKBB.no_adjustment.metasoft_output.extra_cols.txt > output/AD.AOU_${ANCESTRY_INDEX}.UKBB.no_adjustment.metasoft_output.extra_cols.cleaned.txt
